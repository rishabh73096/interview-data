...
const HomePage: React.FC = () => {
  return (
    <main className="mx-auto max-w-6xl px-4 sm:px-6">
      ...
      <section className="pb-10 sm:pb-14">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
          {statTiles.map((tile) => (
            <div
              key={tile.label}
              className="rounded-xl border border-[#9a3412]/12 bg-[#f0e7d6]/55 px-4 py-4 text-center dark:border-white/10 dark:bg-[#f97316]/8"
            >
              <p className="bg-linear-to-br from-[#c2410c] via-[#ea580c] to-[#fb923c] bg-clip-text text-3xl font-bold text-transparent sm:text-4xl">
                {tile.value}
              </p>
      ...
      <section className="pb-10 sm:pb-14">
        <div className="rounded-xl border border-[#9a3412]/12 bg-[#f0e7d6]/55 p-5 sm:p-7 dark:border-white/10 dark:bg-[#f97316]/8">
            ...
            {sections.map((section) => {
              ...
              return (
                <div key={section.href} className="flex flex-col gap-1.5">
                  ...
                  <div className="h-3 w-full rounded-full bg-[#ea580c]/14 dark:bg-[#fb923c]/12">
                    <div
                      className="h-3 rounded-full bg-linear-to-r from-[#c2410c] to-[#fb923c]"
        ...
        <div className="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
          {sections.map((section) => (
            <a
              key={section.href}
              href={section.href}
              className="group flex flex-col gap-4 rounded-xl border border-[#9a3412]/12 bg-[#f0e7d6]/55 p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md sm:p-7 dark:border-white/10 dark:bg-[#f97316]/8"
            >
              <div className="flex items-center justify-between">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-linear-to-br from-[#c2410c] via-[#ea580c] to-[#fb923c] text-white shadow-sm">
                  {section.icon}
                </span>
              ...
              <div>
                <h2 className="mb-1.5 flex items-center gap-1.5 text-lg font-semibold text-gray-900 sm:text-xl dark:text-white">
                  {section.title}
                  <span className="text-gray-400 transition-transform group-hover:translate-x-1 group-hover:text-[#ea580c]">
                    →
                  </span>
                </h2>
        ...
        <div className="grid grid-cols-1 gap-4 sm:gap-0 sm:grid-cols-2 md:grid-cols-4">
          {steps.map((step, idx) => (
            <div key={step.n} className="relative flex flex-col items-center gap-3 px-4 text-center">
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-[#c2410c] via-[#ea580c] to-[#fb923c] text-sm font-bold text-white shadow-sm">
                {step.n}
              </span>
              ...
              {idx < steps.length - 1 && (
                <span className="hidden text-2xl leading-none text-[#fb923c]/60 md:absolute md:top-4 md:-right-2 md:block dark:text-[#f97316]/40">
                  →
                </span>
              )}
            </div>
